                ...
                for update in msg.json()['M']:
                    ...
                    if update_type == 'updateFutureMarketsInfo':
                        ...
                        for contract in update['A'][0]:
                            ...
                            # ساعت 10 تا 12:30
                            ###########################################################################################################################
                            location = 'F:\\برنامه هاي نوشته با پايتون\\aiohttp project\\cdn\\c_n.xlsx'
                            ...
                            with pd.ExcelFile(location) as book:
                                ...
                                sheet = pd.read_excel(book, 'آتي (update_type = 1)', usecols=[0, 1])

                                # ليست نام تمام قرارداد هاي موجود
                                full_names_list = sheet.iloc[:, 0].values
                                # ممد
                                # چك كردن وجود نام قرارداد گرفته شده در ليست قرارداد هاي كدگذاري شده
                                if contract['ContractDescription'] in full_names_list:
                                    # يافتن كد مربوطه و افزودن آن به ديكشنري قرارداد
                                    name_code = sheet.loc[sheet['full_name'] == contract['ContractDescription']][
                                        'name_code'].values[0].item()
                                    ...
                                    sheet.append(pd.DataFrame({'full_name': contract['ContractDescription'],
                                                               'name_code': [last_name_code + 1]})).to_excel(location,
                                                                                                             sheet_name,
                                                                                                             index=False)
                                ...
                                filter = {'_id': 0, 'commodity': 0, 'update_type': 0, 'datetime': 0,
                                          'PersianOrdersDateTime': 0}

                                # جداكردن اخرين قراردادي با اين نام كه در ديتابيس ثبت شده
                                last_doc_filtered = updateFutureMarketsInfo_col.find(query, filter).sort('datetime',
                                                                                                         -1).limit(1)

                                # مجبوريم آنرا به ليست تبديل كنيم
                                # چون در حالت عادي نياز به ساخت حلقه براي يافت ديكشنري هاي درونش است
                                # و ما فقط يك ديكشنري درون اين داريم و نياز به حلقه نيست
                                last_document = list(last_doc_filtered)

                                # ساخت يك كپي از قرارداد تازه رسيده،
                                # و حذف كردن فيلد زمان كه هميشه متغير هست 
                                # (چون دليلي بر آمدن ديتاي جديد نيست)
                                new_con = contract.copy()
                                new_con.pop('PersianOrdersDateTime')

                                # اگر از آن نوع قرارداد قبلا هم در ديتابيس موجود بوده است
                                if last_document != []:
                                    ...
                                    # قرارداد تكراري
                                        ...
                                        confirmation = {'market': 'آتي', 'time': jalaly_datetime}
                                        update_confirmation.insert_one(confirmation)

                                # اگر قراردادي براي اولين بار رسيد
                                elif last_document == []:
